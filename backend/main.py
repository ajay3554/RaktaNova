...
def favicon():
    ...
    return FileResponse(
        PROJECT_ROOT / "frontend" / "icon.png",
...
def register_donor(
    ...
    try:
        payload = DonorRegisterRequest.model_validate(donor_data)
    except ValidationError as exc:
        raise HTTPException(
            status_code=400,
            detail=first_validation_message(exc)
        )
    ...
    existing_donor = (
        db.query(Donor)
        .filter(Donor.phone == payload.phone)
        .first()
    )
...
def donor_login(
    ...
    try:
        payload = DonorLoginRequest.model_validate(login_data)
    except ValidationError as exc:
        raise HTTPException(
            status_code=400,
            detail=first_validation_message(exc)
