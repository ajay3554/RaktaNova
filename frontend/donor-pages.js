...
async function registerDonorPage() {
    ...
    const nameInput =
        document.getElementById("donor-name");

    const ageInput =
        document.getElementById("donor-age");

    const bloodGroupInput =
        document.getElementById("donor-blood-group");

    const phoneInput =
        document.getElementById("donor-phone");

    const passwordInput =
        document.getElementById("donor-password");

    const cityInput =
        document.getElementById("donor-city");

    const nameResult =
        validatePersonName(nameInput?.value);

    const ageResult =
        validateDonorAge(ageInput?.value);

    const phoneResult =
        validateIndianMobile(phoneInput?.value);
    ...
    if (!nameResult.ok) {
        setFieldError(nameInput, nameResult.message);
        hasError = true;
    } else {
    ...
    if (!ageResult.ok) {
        setFieldError(ageInput, ageResult.message);
        hasError = true;
    } else {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            if (
                errorMessage === DUPLICATE_MOBILE_ERROR ||
...
async function donorLoginPage() {
    ...
    const phoneInput =
        document.getElementById("donor-login-phone");
